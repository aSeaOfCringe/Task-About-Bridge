...
public class Main {
    ...
    static void addNode(Node node, int speedA, int speedB, int speedC, boolean goingBack){
        ...
        if (goingBack==false){
            ...
            if (node.getNext1()!=null) addNode(node.next1, speedA, speedB, speedC, true);
            if (node.getNext2()!=null) addNode(node.next2, speedA, speedB, speedC, true);
            if (node.getNext3()!=null) addNode(node.next3, speedA, speedB, speedC, true);

            if (node.next1==null && node.next2==null && node.next3==null) node.setDeadEnd(true);
        }
        else {
            ...
            if (node.getNext2()!=null) addNode(node.next2, speedA, speedB, speedC, false);
